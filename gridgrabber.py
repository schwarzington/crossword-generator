...
class CharBlock(object):
	def __init__(self):
		...
		self.value = "-"
	def __repr__(self):
		return str(self.targetChar)

url = 'http://www.onlinecrosswords.net/layout-'
for i in range(1, 5201):
	newUrl = "grids/layout-" + str(i) + ".png"
	layUrl = "layouts/layout-" + str(i) + ".grid"
	img = cv2.imread(newUrl)    
	gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)    
	ret, thresh = cv2.threshold(gray,127,255,cv2.THRESH_BINARY_INV)
	thresh2 = cv2.bitwise_not(thresh)

	new_img,contours,hierarchy = cv2.findContours(thresh, cv2.RETR_LIST, 1)
	...
	cross_rect = cv2.resize(cross_rect,(150,150))
	...
	for i in xrange(15):
		for j in xrange(15):
			...
			if cv2.countNonZero(box) > 50:
				cross.itemset((i,j),None)

	with open(layUrl, 'w') as f:
		print >> f, cross
